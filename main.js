...
__export(main_exports, {
  default: () => EmbeddedNoteEnhancerPlugin
});
...
var EmbeddedNoteEnhancerPlugin = class extends import_obsidian.Plugin {
  ...
  processEmbeddedBlock(block) {
    ...
    if (block.tagName.toLowerCase() !== "div" && block.tagName.toLowerCase() !== "span") {
      if (this.debugVerbose)
        console.log(`[EmbeddedNoteEnhancer] Skipping non-div/span element:`, block.tagName, block);
      return;
    }
    if (block.tagName.toLowerCase() === "span") {
      ...
      if (!hasEmbedContent) {
        if (this.debugVerbose)
          console.log(`[EmbeddedNoteEnhancer] Skipping span without embed content:`, block);
        return;
      }
    }
  ...
  async disableInlineEditing(embedContent) {
    ...
    if (blockEl) {
      ...
      if (this.debugVerbose)
        console.debug("[EmbeddedNoteEnhancer] unfreeze on", blockEl.getAttribute("data-block-id"));
    }
  }
  /**
   * 处理文件修改事件
  ...
  async enableInlineEditing(block) {
    ...
    if (this.debugVerbose)
      console.debug("[EmbeddedNoteEnhancer] freeze on", block.getAttribute("data-block-id"));
    block.setAttribute("data-freeze", "true");
...
var EmbeddedNoteEnhancerSettingTab = class extends import_obsidian.PluginSettingTab {
